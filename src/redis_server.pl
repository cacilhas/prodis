...
process("APPEND", [Key, Value], R) :-
    ...
    p_append(Value, Current, NewValue),
    regset(Key, NewValue),
    string_length(NewValue, Length),
...
p_append(Value, Current, R) :-
    is_list(Value), !,
    string_codes(Value1, Value),
    p_append(Value1, Current, R).

p_append(Value, Current, R) :-
    atom(Current), !,
    atom_string(Current, Current1),
    p_append(Value, Current1, R).

p_append(Value, Current, R) :-
    number(Current), !,
    term_string(Current, Current1),
    p_append(Value, Current1, R).

p_append(Value, Current, R) :-
    is_list(Current), !,
    string_codes(Current1, Current),
    append(Current1, Value, R).

p_append(Value, Current, R) :-
    string(Value), string(Current), !,
    string_concat(Current, Value, R).
